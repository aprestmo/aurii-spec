...
export class PostgresAdapter implements StorageAdapter {
  ...
  async listDatasets(): Promise<Dataset[]> {
    const rows = await this.sql`SELECT * FROM aurii_datasets ORDER BY created_at ASC`;
  ...
  async listSchemas(datasetId?: string): Promise<StoredSchema[]> {
    const rows = datasetId
      ? await this.sql`SELECT * FROM aurii_schemas WHERE dataset_id = ${datasetId} ORDER BY created_at DESC`
  ...
  async getEntity(id: string): Promise<Entity | null> {
    const rows = await this.sql`SELECT * FROM aurii_entities WHERE id = ${id}::uuid`;
    return rows[0] ? rowToEntity(rows[0] as RawEntityRow) : null;
  }

  async listEntities(schemaId: string, datasetId: string, limit = 50, offset = 0): Promise<Entity[]> {
    const rows = await this.sql`
  ...
  async updateImportRun(id: string, patch: Partial<ImportRunRecord>): Promise<void> {
    ...
    if (patch.errors !== undefined) push("errors", JSON.stringify(patch.errors));
  ...
  async listImportRuns(datasetId?: string, limit = 20): Promise<ImportRunRecord[]> {
    const rows = datasetId
      ...
      : await this.sql`SELECT * FROM aurii_import_runs ORDER BY created_at DESC LIMIT ${limit}`;
    ...
    return rows.map((r: Record<string, unknown>) => ({
      ...
      completedAt: r["completed_at"] ? ts(r["completed_at"] as string | Date) : null,
      createdAt: ts(r["created_at"] as string | Date),
    }));
  }
