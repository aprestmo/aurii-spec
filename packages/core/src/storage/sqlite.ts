...
function conditionToSql(condition: Condition, params: unknown[]): string {
  ...
  const value =
    typeof condition.value === "boolean" ? (condition.value ? 1 : 0) : condition.value;
  params.push(value);
...
export class SqliteAdapter implements StorageAdapter {
  ...
  constructor(path?: string) {
    const dbPath = path ?? process.env["AURII_DB_PATH"] ?? join(process.cwd(), "aurii.db");
  ...
  async getDataset(id: string): Promise<Dataset | null> {
    const row = this.db.prepare("SELECT * FROM aurii_datasets WHERE id = ?").get(id) as
  ...
  async listDatasets(): Promise<Dataset[]> {
    const rows = this.db.prepare("SELECT * FROM aurii_datasets ORDER BY created_at ASC").all() as {
      id: string;
      name: string;
      description: string | null;
      created_at: string;
  ...
  async upsertSchema(def: SchemaDefinition, datasetId: string): Promise<StoredSchema> {
    ...
    this.db
      ...
      .run(def.id, datasetId, def.name, def.description ?? null, def.version ?? 1, JSON.stringify(def), now, now);
  ...
  async listSchemas(datasetId?: string): Promise<StoredSchema[]> {
    const rows = (
      datasetId
        ? this.db.prepare("SELECT * FROM aurii_schemas WHERE dataset_id = ? ORDER BY created_at DESC").all(datasetId)
        : this.db.prepare("SELECT * FROM aurii_schemas ORDER BY created_at DESC").all()
    ) as RawSchemaRow[];
  ...
  async insertEntities(inputs: EntityInput[], datasetId: string): Promise<Entity[]> {
    const now = new Date().toISOString();
    ...
    const insertMany = this.db.transaction((rows: EntityInput[]) => {
      ...
      for (const input of rows) {
        ...
        insert.run(id, datasetId, input.schemaId, JSON.stringify(input.data), input.state ?? "active", now, now);
        ids.push(id);
      }
      return ids;
    });
  ...
  async getEntity(id: string): Promise<Entity | null> {
    const row = this.db.prepare("SELECT * FROM aurii_entities WHERE id = ?").get(id) as RawEntityRow | null;
    return row ? rowToEntity(row) : null;
  }

  async listEntities(schemaId: string, datasetId: string, limit?: number, offset?: number): Promise<Entity[]> {
  ...
  async countEntities(schemaId: string, datasetId: string): Promise<number> {
    const row = this.db
      .prepare("SELECT COUNT(*) as count FROM aurii_entities WHERE dataset_id = ? AND schema_id = ?")
  ...
  async recordImportRun(run: Omit<ImportRunRecord, "createdAt">): Promise<void> {
    this.db
      .prepare(
  ...
  async updateImportRun(id: string, patch: Partial<ImportRunRecord>): Promise<void> {
    const sets: string[] = [];
    ...
    params.push(id);
    this.db.prepare(`UPDATE aurii_import_runs SET ${sets.join(", ")} WHERE id = ?`).run(...params);
  }

  async listImportRuns(datasetId?: string, limit = 20): Promise<ImportRunRecord[]> {
    const rows = (
      datasetId
        ? this.db
            .prepare("SELECT * FROM aurii_import_runs WHERE dataset_id = ? ORDER BY created_at DESC LIMIT ?")
            .all(datasetId, limit)
        : this.db.prepare("SELECT * FROM aurii_import_runs ORDER BY created_at DESC LIMIT ?").all(limit)
  ...
  async getStats(datasetId: string): Promise<StorageStats> {
    ...
    for (const schema of schemas) {
      ...
      const fieldCoverage = schema.fields.map((f) => {
        ...
        return {
          field: f.name,
          pct: sample.length === 0 ? 0 : Math.round((populated / sample.length) * 100),
        };
      });

      schemaStats.push({ schemaId: schema.id, name: schema.name, count, fieldCoverage });
    }
