 ...
 *
 * These run the *exact same contract* as `sqlite-adapter.test.ts` against a
 * real PostgreSQL/JSONB database — SQLite and PostgreSQL differ in JSON
 * querying, numeric/boolean casting, ordering and transaction behaviour, so
 * "verified on SQLite" does not imply "verified on PostgreSQL".
 *
 * Requires a reachable `DATABASE_URL` (e.g. started via `docker compose` or
 * the `postgres:` service container in CI). When it is not set — e.g. a
 * contributor's machine without Docker — the whole suite is skipped rather
 * than failing, so `bun test` still works out of the box.
 */
...
/**
 * Tests run against a real, persistent database rather than a fresh
 * :memory: file, so each test starts from a clean slate explicitly.
 */
...
describe.skipIf(!DATABASE_URL)("PostgresAdapter", () => {
	let db: PostgresAdapter;

	beforeAll(async () => {
		// Fail fast with a clear message rather than a cryptic connection error
		// buried inside the first test if the URL is set but unreachable.
		const probe = new PostgresAdapter(DATABASE_URL);
		await probe.init();
		await probe.close();
	});
	...
	beforeEach(async () => {
		...
		db = new PostgresAdapter(DATABASE_URL);
		await db.init();
	});
