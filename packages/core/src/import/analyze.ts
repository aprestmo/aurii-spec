...
const BOOL_VALUES = new Set(["true", "false", "0", "1", "yes", "no", "ja", "nei", "y", "n"]);
const DATE_RES = [
  /^\d{4}-\d{2}-\d{2}/,
  /^\d{1,2}\/\d{1,2}\/\d{4}$/,
  /^\d{1,2}\.\d{1,2}\.\d{4}$/,
...
function inferType(values: string[]): FieldType {
  ...
  if (nonEmpty.every((v) => !isNaN(Number(v.replace(",", "."))) && v.trim() !== "")) {
    return "number";
  }
...
export function analyzeContent(
  ...
  const schemaId =
    suggestedId ??
    slugifyFieldName(filename.replace(/\.(csv|json)$/i, "")).toLowerCase().replace(/[^a-z0-9-]/g, "-");
