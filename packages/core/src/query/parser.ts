...
const KEYWORDS = new Set([
  "from", "select", "where", "and", "order", "by",
...
function tokenize(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    if (/[a-zA-Z_]/.test(ch)) {
      ...
      if (lower === "true")  { tokens.push({ kind: "boolean", value: true,  raw: word }); continue; }
      if (lower === "false") { tokens.push({ kind: "boolean", value: false, raw: word }); continue; }
      if (lower === "null")  { tokens.push({ kind: "null",    value: null,  raw: word }); continue; }
      if (lower === "contains") { tokens.push({ kind: "op", value: "contains", raw: word }); continue; }
      if (KEYWORDS.has(lower))  { tokens.push({ kind: "keyword", value: lower, raw: word }); continue; }
...
class Parser {
  ...
  private expect(kind: TokenKind, value?: string): Token {
    const tok = this.peek();
    if (tok.kind !== kind) {
      throw new Error(`Expected ${kind}${value ? ` "${value}"` : ""} but got ${tok.kind} ("${tok.raw}")`);
    }
  ...
  parse(): ParsedQuery {
    ...
    while (!this.match("eof")) {
      if (this.match("keyword", "select")) {
        ...
        const tok = this.peek();
        throw new Error(`Unexpected token "${tok.raw}" at position ${this.pos}`);
      }
    }
  ...
  private parseCondition(): Condition {
    ...
    if (!["string", "number", "boolean", "null"].includes(tok.kind)) {
      throw new Error(`Expected a value after operator but got ${tok.kind} ("${tok.raw}")`);
    }
